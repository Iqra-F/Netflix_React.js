...
const SavedMovies = () => {
  ...
  useEffect(() => {
    //we run this useeffect everytime the snapshot changes
    onSnapshot(
          ...
          {movies.map((item, id) => (
            ...
            >
              ...
              <div className="absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-100 text-white">
                ...
                >
                   <AiOutlineClose /> {/*// whenever we delete an object from an array in firebase, we have to do it on the client side,
      // we have to remove it from our state on the client side  and then push it back to firebase, 
      //the updated array without id. that is why we are passing id(item.id) here when we click this delete btn.*/}
                </p>
              </div>
            </div>
          ))}
